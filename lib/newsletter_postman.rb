  ...
  def perform
    ...
    @issue.newsletter.newsletter_subscriptions.each do |subscription|
      if NewsletterBlacklist.find_by_mail(subscription.mail).nil?  && @issue.newsletter_deliveries.find(:all,
        ...
        begin
          prepared_url = "<a href='#{ROOT_URL}/subscriptions/"+
            encode64(subscription.mail)+
            "/"+
            subscription.token+
          ...
          UserMailer.deliver_newsletter(
            ...
            subject, 
            header_image_tag, 
            header.gsub(/SUBSCRIPTION_URL/,prepared_url  
                   ).gsub(/NEWLETTER_MAIL/, subscription.mail
                   ).gsub(/BLOCK_MAIL_URL/, prepared_url
                   ), 
            footer.gsub(/SUBSCRIPTION_URL/, prepared_url
                   ).gsub(/NEWLETTER_MAIL/, subscription.mail
                   ).gsub(/BLOCK_MAIL_URL/, prepared_url
                   ), 
            body, 
